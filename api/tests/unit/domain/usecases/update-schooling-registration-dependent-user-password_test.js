...
const {
  UserNotAuthorizedToUpdateStudentPasswordError, UserNotFoundError, UserNotAuthorizedToUpdatePasswordError
...
describe('Unit | UseCase | update-schooling-registration-dependent-user-password', () => {
  ...
  beforeEach(() => {
    ...
    userRepository = {
      ...
      updateTemporaryPassword: sinon.stub().resolves()
    };
  ...
  it('should update user password with a hashed password', async () => {
    ...
    expect(userRepository.updateTemporaryPassword).to.have.been.calledWith(userStudent.id, encryptedPassword);
  });
  ...
  describe('When update user student\'s password is not possible', () => {
    ...
    it('should return a UserNotAuthorizedToUpdatePasswordError when user student have no username and email', async () => {
      // given
